...
void proc_clients(pool* p)
{
    ...
    for (int i = 0; (i <= p->maxci) && (p->nready > 0); i++) {
        ...
        if ((connfd > 0) && (FD_ISSET(connfd, &(p->ready_set)))) {
            p->nready--;
            NO_EINTR(rn = recv(connfd, p->buffers[i], BUF_SIZE, 0));

            if (rn >= 1) {
                NO_EINTR(sn = send(connfd, p->buffers[i], rn, 0));
                /* wrong when sn != rn */
                if (sn != rn) {
...
int main(int argc, char* argv[])
{
    ...
    while (1) {
        ...
        /* restart from EINTR */
        NO_EINTR(pool.nready = select(pool.maxfd + 1, 
                ...
                &(pool.ready_set))) // this socket is ready to be accepted
        {
            ...
            NO_EINTR(client_sock = accept(sock, (struct sockaddr*)&cli_addr, &cli_size));
            if (client_sock == -1)
            {
                close_all_clients(&pool);
                close_socket(sock);
